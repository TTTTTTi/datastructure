...
int getValue(string profix)
{
    stack<int> test;
    for(int i=0;i<profix.size();i++)
    {
        if(profix[i]>='0'&&profix[i]<='9')
        {
            test.push(profix[i]-'0');
            //cout<<"push"<<test.top()<<endl;
        }
        else
        {
            // if(test.empty()||profix[i]=='('||profix[i]==')')
            // {
            ...
            // {
                ...
                switch(profix[i])
                {
                    ...
                    case '*':x3=x2*x1;break;
                    case '/':x3=x2/x1;break;
                    case 'm':x3=x2%x1;break;
                    //default:cout<<"sign error!"<<endl;return -1;
                }
                test.push(x3);
                //cout<<"push"<<test.top()<<endl;;
            //}
